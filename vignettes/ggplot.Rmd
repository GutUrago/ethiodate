...
```{r, echo=FALSE}
# To save plots of Amharic fonts
...
p1 <- cpieth |> 
  ...
  theme_bw() + 
  theme(axis.text.x = element_text(angle = 45, vjust = 0.9, hjust = 1, face = "bold"))
...
```{r, include=FALSE}
ggsave("figures/p1.png", plot = last_plot(), device = "png", dpi = 300)
```


![](figures/p1.png){width=100%}
...
```{r, include=FALSE}
ggsave("figures/p2.png", plot = last_plot(), device = "png", dpi = 300)
```


![](figures/p2.png){width=100%}
